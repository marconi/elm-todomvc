...
Elm.Todo.make = function (_elm) {
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var getFooterInfo = A2($Html.footer,
   ...
   _U.list([A2($Html.p,
           ...
           _U.list([$Html.text("Written by ")
                   ,A2($Html.a,
                   _U.list([$Html$Attributes.href("https://github.com/marconi")]),
                   _U.list([$Html.text("Marconi Moreto")]))]))
           ,A2($Html.p,
           _U.list([]),
   ...
   var onTodoInput = F2(function (address,action) {
      return A3($Html$Events.on,
      "input",
      $Html$Events.targetValue,
      function (value) {
         return A2($Signal.message,address,action(value));
      });
   });
   var Completed = {ctor: "Completed"};
   var Active = {ctor: "Active"};
   var All = {ctor: "All"};
   var SelectFilter = function (a) {
      return {ctor: "SelectFilter",_0: a};
   };
   var getFilters = F2(function (address,appliedFilter) {
      var filterItem = function (filter) {
         return A2($Html.li,
         _U.list([]),
         _U.list([A2($Html.a,
         _U.list([$Html$Attributes.href($String.toLower(A2($Basics._op["++"],
                 "#",
                 $Basics.toString(filter))))
                 ,$Html$Attributes.classList(_U.list([{ctor: "_Tuple2"
                                                      ,_0: "selected"
                                                      ,_1: _U.eq(appliedFilter,filter)}]))
                 ,A2($Html$Events.onClick,address,SelectFilter(filter))]),
         _U.list([$Html.text($Basics.toString(filter))]))]));
      };
      return A2($Html.ul,
      _U.list([$Html$Attributes.id("filters")]),
      _U.list([filterItem(All)
              ,filterItem(Active)
              ,filterItem(Completed)]));
   });
   var getFooter = F4(function (address,
   leftCounter,
   completedCounter,
   appliedFilter) {
      ...
      _U.list([A2($Html.span,
              ...
              _U.list([A2($Html.strong,
                      _U.list([]),
                      _U.list([$Html.text($Basics.toString(leftCounter))]))
                      ,$Html.text(" item left")]))
              ,A2(getFilters,address,appliedFilter)
              ,A2($Html.button,
              ...
              "Clear completed (",
              A2($Basics._op["++"],
              $Basics.toString(completedCounter),
              ")")))]))]));
   });
   var ToggleCompletedAll = {ctor: "ToggleCompletedAll"};
   var Delete = function (a) {    return {ctor: "Delete",_0: a};};
   var ToggleCompleted = function (a) {
      return {ctor: "ToggleCompleted",_0: a};
   };
   var getTodoItem = F2(function (address,todo) {
      return A2($Html.li,
      ...
      _U.list([A2($Html.div,
              ...
              _U.list([A2($Html.input,
                      _U.list([$Html$Attributes.$class("toggle")
                              ...
                              ,$Html$Attributes.checked(todo.isCompleted)
                              ,A2($Html$Events.onClick,address,ToggleCompleted(todo.id))]),
                      _U.list([]))
                      ,A2($Html.label,_U.list([]),_U.list([$Html.text(todo.name)]))
                      ,A2($Html.button,
   ...
   var getTodos = F3(function (address,appliedFilter,todos) {
      var filterTodo = function (todo) {
         return _U.eq(appliedFilter,
         Active) && $Basics.not(todo.isCompleted) ? true : _U.eq(appliedFilter,
         Completed) && todo.isCompleted ? true : _U.eq(appliedFilter,
         All) ? true : false;
      };
      var filteredTodos = A2($List.filter,filterTodo,todos);
      var todoItems = A2($List.map,
      getTodoItem(address),
      filteredTodos);
      return A2($Html.ul,
   ...
   var UpdateTodoInput = function (a) {
      return {ctor: "UpdateTodoInput",_0: a};
   };
   var Add = {ctor: "Add"};
   var NoOp = {ctor: "NoOp"};
   var onTodoEnter = F2(function (address,action) {
      var keycodeAction = function (code) {
         return _U.eq(code,13) ? Add : NoOp;
      };
      return A3($Html$Events.on,
      "keyup",
      $Html$Events.keyCode,
      function (code) {
         return A2($Signal.message,address,keycodeAction(code));
      });
   });
   var getHeader = F2(function (address,todoInput) {
      ...
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("todos")]))
              ...
              _U.list([$Html$Attributes.type$("text")
                      ...
                      ,$Html$Attributes.value(todoInput)
                      ,A2(onTodoInput,address,UpdateTodoInput)
                      ,A2(onTodoEnter,address,Add)]),
              _U.list([]))]));
   });
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.section,
              ...
              _U.list([A2(getHeader,address,model.todoInput)
                      ,A2($Html.section,
                      ...
                      _U.list([A2($Html.input,
                              _U.list([$Html$Attributes.id("toggle-all")
                                      ...
                                      ,A2($Html$Events.onClick,address,ToggleCompletedAll)]),
                              _U.list([]))
                              ,A2($Html.label,
                              _U.list([$Html$Attributes.$for("toggle-all")]),
                              _U.list([$Html.text("Mark all as complete")]))
                              ,A3(getTodos,address,model.appliedFilter,model.todos)]))
                      ,A4(getFooter,
                      address,
                      model.leftCounter,
                      model.completedCounter,
                      model.appliedFilter)]))
              ,getFooterInfo]));
   });
   var initialModel = {todos: _U.list([])
                      ,leftCounter: 0
                      ,appliedFilter: All
                      ,completedCounter: 0
                      ,todoInput: ""
   ...
   var newTodo = F3(function (id,name,isCompleted) {
      return {id: id,name: name,isCompleted: isCompleted};
   });
   var update = F2(function (action,model) {
      return A3($Debug.watchSummary,
      "update action",
      function (_p0) {
         return {ctor: "_Tuple2",_0: action,_1: model};
      },
      function () {
         var _p1 = action;
         switch (_p1.ctor)
         {case "NoOp": return model;
            case "Add": var todo = A3(newTodo,
              model.nextId,
              model.todoInput,
              false);
              return _U.update(model,
              {todos: A2($List._op["::"],todo,model.todos)
              ,todoInput: ""
              ,leftCounter: model.leftCounter + 1
              ,nextId: model.nextId + 1});
            case "UpdateTodoInput": return _U.update(model,
              {todoInput: _p1._0});
            case "ToggleCompletedAll":
            var isCompletedAll = $Basics.not(model.isCompletedAll);
              var todos = A2($List.map,
              function (todo) {
                 return _U.update(todo,{isCompleted: isCompletedAll});
              },
              model.todos);
              var todosCounter = $List.length(todos);
              return _U.update(model,
              {isCompletedAll: isCompletedAll
              ,todos: todos
              ,leftCounter: isCompletedAll ? 0 : todosCounter
              ,completedCounter: isCompletedAll ? todosCounter : 0});
            case "ToggleCompleted": var _p2 = _p1._0;
              var toggleCompleted = function (todo) {
                 return _U.eq(todo.id,_p2) ? _U.update(todo,
                 {isCompleted: $Basics.not(todo.isCompleted)}) : todo;
              };
              var toggledTodos = A2($List.filter,
              function (todo) {
                 return _U.eq(todo.id,_p2);
              },
              model.todos);
              var isTodoCompleted = _U.cmp($List.length(A2($List.filter,
              function (_) {
                 return _.isCompleted;
              },
              toggledTodos)),
              0) > 0;
              var leftCounter = isTodoCompleted ? model.leftCounter + 1 : model.leftCounter - 1;
              var completedCounter = isTodoCompleted ? model.completedCounter - 1 : model.completedCounter + 1;
              return _U.update(model,
              {todos: A2($List.map,toggleCompleted,model.todos)
              ,leftCounter: leftCounter
              ,completedCounter: completedCounter});
            case "Delete": var nonDeletedTodos = A2($List.filter,
              function (todo) {
                 return !_U.eq(todo.id,_p1._0);
              },
              model.todos);
              var completedCounter = $List.length(A2($List.filter,
              function (_) {
                 return _.isCompleted;
              },
              nonDeletedTodos));
              var leftCounter = $List.length(nonDeletedTodos) - completedCounter;
              return _U.update(model,
              {todos: nonDeletedTodos
              ,leftCounter: leftCounter
              ,completedCounter: completedCounter});
            default: return _U.update(model,{appliedFilter: _p1._0});}
      }());
   });
   ...
   var Todo = F3(function (a,b,c) {
      return {id: a,name: b,isCompleted: c};
   });
